...
class _AddNewBabyState extends State<AddNewBaby> {
  ...
  _mSelectDate(BuildContext context) async {
    selected = startDate;
    DateTime? firstDate =
        DateTime.now().add(const Duration(days: -(6 * 365 - 1)));
  ...
  void mActions() async {
    ...
    if (_editingControllerBabyName.text.isNotEmpty &&
      ...
      await MySqfliteServices.mAddBabyInfo(
              ...
              gender: gender,
              headCircumstance: _editingControllerHeadCircumstance
                      .text.isNotEmpty
          ...
          .then((babyId) async {
        if (babyId >= 0) {
          int runningday = MyServices.mGetRunningDays(dob: selected!);

          /* await MySqfliteServices.mFetchCurrentBabyWeightHeightTable(
              widget.email, widget.momId, babyId); */

          await _mUpdateStartingWeightAndHeightIntoDb(
                  runningday, widget.momId, widget.email, babyId)
              .then((value) async {
            if (value) {
              AwesomeDialog(
                context: context,
  ...
  Future<bool> _mUpdateStartingWeightAndHeightIntoDb(
      int runningday, int momId, String email, int babyId) async {
    bool isUpdated = false;
    Map<String, dynamic> mapResult =
        MyServices.mGenerateBabyWeekMonthNo(babyCurrentAgeInDay: runningday);
    BabyWeekMonthNo babyWeekMonthNo = mapResult[MyKeywords.intoEngFontModel][0];

    isUpdated = await MySqfliteServices.mUpdateBabyWeight(
        email: email,
        momId: momId,
        babyId: babyId,
        ageTag: babyWeekMonthNo.text,
        ageNum: int.parse(babyWeekMonthNo.num),
        weight: int.parse(_kgCtrller.text));

    isUpdated = await MySqfliteServices.mUpdateBabyHeight(
        email: email,
        momId: momId,
        babyId: babyId,
        ageTag: babyWeekMonthNo.text,
        ageNum: int.parse(babyWeekMonthNo.num),
        height: int.parse(_cmCtrller.text));
