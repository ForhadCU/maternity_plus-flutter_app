...
class MySqfliteServices {
  static Future<Database> dbInit() async {
    ...
    return await openDatabase(
      ...
      onCreate: (db, version) async {
        ...
        await db.execute(
            "CREATE TABLE ${MyKeywords.babygrowthQuesTable} (id INTEGER PRIMARY KEY AUTOINCREMENT, ques_id TEXT, question TEXT, timestar NUMBER, options TEXT)");
        ...
        await db.execute(
            "CREATE TABLE ${MyKeywords.babygrowthResponseTable} (${MyKeywords.email} TEXT, ${MyKeywords.momId} NUMBER, ${MyKeywords.baby_id} NUMBER, ${MyKeywords.quesId} TEXT, ${MyKeywords.answerStatus} NUMBER, ${MyKeywords.inputedAnswer} TEXT )");
  ...
  static Future<int> mAddBabyInfo(
    ...
    weightsList.insert(0, 0.toString());
    heightsList.insert(0, 0.toString());
    var ageNumList = MyServices.mProduceWeekMonthPatternForBabyWeightHeight();
    ...
    logger.d("Baby id before Save into WeightHeight Table: $babyId");
...
/* 
      var res = await db.rawQuery(
          "SELECT * FROM ${MyKeywords.babyweightsAndHeightsTable} WHERE ${MyKeywords.email} =  ? & ${MyKeywords.momId} = ? & ${MyKeywords.baby_id}",
          [email, momId, babyId]);

      logger.w("Baby Weight Height Table");
 ...
 */
  ...
  static Future<bool> mUpdateBabyWeight(
      {required String email,
      required int momId,
      required int babyId,
      required String ageTag,
      required int ageNum,
    ...
    int i = await db.rawUpdate(
        "UPDATE ${MyKeywords.babyweightsAndHeightsTable} SET ${MyKeywords.babyWeight} =  ? WHERE ${MyKeywords.email} = ? AND ${MyKeywords.momId} = ? AND ${MyKeywords.baby_id} = ? AND ${MyKeywords.ageTag} = ? AND ${MyKeywords.ageNum} = ? ",
        [weight, email, momId, babyId, ageTag, ageNum]);
    logger.d("Update number i : $i");
...
/*         ageTag,
  ...
  static Future<bool> mUpdateBabyHeight(
    ...
    int i = await db.rawUpdate(
        "UPDATE ${MyKeywords.babyweightsAndHeightsTable} SET ${MyKeywords.babyHeight} =  ? WHERE ${MyKeywords.email} = ? AND ${MyKeywords.momId} = ? AND ${MyKeywords.baby_id} = ? AND ${MyKeywords.ageTag} = ? AND ${MyKeywords.ageNum} = ? ",
        [height, email, momId, babyId, ageTag, ageNum]);
    logger.d("Update number i : $i");
...
/*   
    static Future<bool> mUpdateBabyHeight({required int momId, required String email, required int babyId, required String ageTag, required int ageNum, required int height})  */
  ...
  static Future<void> mUpdateAnswerStatus(
      ...
      required String inputedAnswer}) async {
    final db = await MySqfliteServices.dbInit();
    logger.d("Inputed Ans in UpdateFunc: $inputedAnswer");
    var r = await db.rawUpdate(
        "UPDATE ${MyKeywords.babygrowthResponseTable} SET ${MyKeywords.answerStatus} = ?, ${MyKeywords.inputedAnswer} = ? WHERE ${MyKeywords.momId} = ? AND ${MyKeywords.email} = ? AND ${MyKeywords.baby_id} = ? AND ${MyKeywords.quesId} = ?",
        [answerStatus, inputedAnswer, momId, email, babyId, quesId]);
    ...
    if (r == 0) {
      ...
      await db.insert(MyKeywords.babygrowthResponseTable, {
        ...
        MyKeywords.inputedAnswer: inputedAnswer,
      });
...
/*   static Future<List<Map<String, Object?>>> mGetCurrentBabyWeightList(
  ...
  static Future<Map<String, dynamic>?> mFetchAnswerStatus(
      {required String email,
      required int momId,
      int? babyId,
    ...
    if (r.isNotEmpty) {
      return {
        MyKeywords.answerStatus:
            int.parse(r.first[MyKeywords.answerStatus].toString()),
        MyKeywords.inputedAnswer: r.first[MyKeywords.inputedAnswer].toString(),
      };
    }
    return null;
  }
  ...
  static Future<void> mFetchCurrentBabyWeightHeightTable(
    String email,
    int momId,
    int babyId,
    /* String ageTag, int ageNum */
  ) async {
    final db = await MySqfliteServices.dbInit();
...
/*     logger.w("email is : $email, babyId is: $babyId");
    var map = await db.rawQuery(
        "SELECT * FROM ${MyKeywords.babyweightsAndHeightsTable} WHERE ${MyKeywords.baby_id} = ? AND ${MyKeywords.email} = ?",
        [babyId, email]); */

    logger.d("Show Weight: ${map[5][MyKeywords.ageNum]}");
  }
}
